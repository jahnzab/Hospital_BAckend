...
def generate_slots(db, doctor_id, pref_date):
    availability = db.query(Availability_of_Doctors).filter(
        Availability_of_Doctors.doctor_id == doctor_id,
        Availability_of_Doctors.date == pref_date
    ).first()
    if not availability:
        return []

    start_time = datetime.combine(pref_date, availability.start_time)
    end_time = datetime.combine(pref_date, availability.end_time)

    # If booking today, skip past time + 20 mins buffer
    now = datetime.now()
    if pref_date == now.date():
        buffer_time = now + timedelta(minutes=20)
        if buffer_time > start_time:
            minute = ((buffer_time.minute // 10) + 1) * 10
            start_time = buffer_time.replace(minute=0, second=0, microsecond=0) + timedelta(minutes=minute)

    # Generate all slots
    all_slots = []
    while start_time <= end_time:
        all_slots.append(start_time.strftime("%I:%M %p"))
        start_time += timedelta(minutes=10)

    # Remove already booked slots
    booked_slots = [p.slot for p in db.query(Patients).filter(
        Patients.doctor_id == doctor_id,
        Patients.preferred_date == pref_date
    ).all()]
...
def chat_endpoint(msg: ChatMessage, db: Session = Depends(get_db)):
    ...
    if suggested_specialization:
        ...
        for delta in range(future_days):
            ...
            for r in rows:
                ...
                slots = generate_slots(db, key, check_date)
                if slots:
                    doctor_dict[key]["dates"][check_date.isoformat()] = slots
        ...
        for d_id, info in doctor_dict.items():
            for d_date, slots in info["dates"].items():
                choices.append(f"- {info['doctor_name']} ({info['specialization']}) | Room {info['room']} | Date: {d_date} | ID: {d_id} | Slots: {', '.join(slots)}")
        return {"reply": "Based on your symptom, here are the available doctors:\n" + "\n".join(choices)}
    ...
    if "list doctor" in ltext or "show doctor" in ltext:
        ...
        for r in rows:
            ...
            slots = generate_slots(db, key, r.date)
            if slots:
                doctor_dict[key]["dates"][r.date.isoformat()] = slots

        if not doctor_dict:
            return {"reply": "No doctors available in the next 7 days."}
    ...
    if state == "choose_specialization":
        ...
        doctors_list = []
        for r in rows:
            slots = generate_slots(db, r.doctor_id, r.date)
            doctors_list.append(f"- {r.doctor.doctor_name} ({r.doctor.specialization}) | Room {r.room_number} | ID: {r.doctor_id} | Slots: {', '.join(slots)}")
    ...
    if state == "choose_doctor":
        ...
        sess["state"] = "collect_name"
        set_session(msg.session_id, sess)
        return {"reply": f"✅ Selected. What's your full name?"}
    ...
    if state == "collect_date":
        ...
        slots = generate_slots(db, sess["data"]["doctor_id"], pref_date)
        if not slots:
            return {"reply": "No slots available for this doctor on this date."}
        return {"reply": "Available slots are: " + ", ".join(slots) + ". Type your preferred slot (e.g., 10:00 AM)."}

    if state == "collect_slot":
        slot = text.strip()
        doctor_id = sess["data"]["doctor_id"]
        pref_date = sess["data"]["preferred_date"]
        valid_slots = generate_slots(db, doctor_id, pref_date)
        ...
        set_session(msg.session_id, sess)
        d = db.query(Doctors).filter(Doctors.doctor_id == doctor_id).first()
        return {"reply": f"Confirm booking for {sess['data']['patient_name']} with Dr. {d.doctor_name} on {pref_date} at {slot}? Reply 'yes' to confirm."}
            ...
            try:
                with db.begin():
                    new_patient, token, _, _ = book_for_doctor(
                        db, doctor_id, pdata, preferred_date=pref_date, slot=slot
                    )
                clear_session(msg.session_id)
                d = db.query(Doctors).filter(Doctors.doctor_id == doctor_id).first()
