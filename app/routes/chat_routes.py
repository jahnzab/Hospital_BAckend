...
def chat_endpoint(msg: ChatMessage, db: Session = Depends(get_db)):
    ...
    if suggested_specialization:
      ...
      rows = db.query(Availability_of_Doctors).join(Doctors).filter(
        ...
        Availability_of_Doctors.date >= today
      ).order_by(Availability_of_Doctors.doctor_id, Availability_of_Doctors.date).all()

      if not rows:
        return {"reply": f"No doctors found for '{suggested_specialization}'. Try another specialization or symptom."}

      # Group by doctor and show each doctor once per row, with date and slots
      doctor_dict = {}
      for r in rows:
        key = r.doctor_id
        ...
        # Generate slots from 10:00 AM to 5:00 PM every 10 minutes
        slot_list = []
      ...
      # Save available doctors in session to allow booking
      sess["data"]["choices"] = doctor_dict
      sess["state"] = "choose_doctor"
      set_session(msg.session_id, sess)

      reply_text = "Based on your input, here are the available doctors:\n" + "\n".join(choices)
